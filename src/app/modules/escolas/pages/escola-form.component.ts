...
@Component({
  ...
  template: `
    <div class="page">
      <div class="header"><div><h1>{{ isEdit ? 'Editar' : 'Nova' }} Escola</h1></div><a routerLink="/escolas" class="btn btn-outline"><span class="material-icons">arrow_back</span></a></div>
      <div class="card"><div class="card-body">
        <div class="form-grid">
          <div class="form-group"><label>Nome *</label><input class="form-control" [(ngModel)]="form.name"></div>
          <div class="form-group"><label>Nível</label><input class="form-control" [(ngModel)]="form.level"></div>
          <div class="form-group full"><label>Localização</label><input class="form-control" [(ngModel)]="form.location"></div>
        ...
        <div class="form-actions"><a routerLink="/escolas" class="btn btn-outline">Cancelar</a><button class="btn btn-primary" (click)="save()" [disabled]="saving()">{{ saving() ? 'Salvando...' : 'Salvar' }}</button></div>
      </div></div>
    </div>
  `,
...
export class EscolaFormComponent implements OnInit {
  private service = inject(EscolasService); private router = inject(Router); private route = inject(ActivatedRoute);
  isEdit = false; id = ''; saving = signal(false);
  form: any = { name: '', level: '', location: '', phone: '', email: '', notes: '' };
  ngOnInit() {
  ...
  save() {
    ...
    obs.subscribe({ next: () => this.router.navigate(['/escolas']), error: () => { this.saving.set(false); alert('Erro ao salvar'); } });
  }
}
