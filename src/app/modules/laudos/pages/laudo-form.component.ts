...
@Component({
  ...
  template: `
    <div class="page">
      <div class="header"><div><h1>{{ isEdit ? 'Editar' : 'Novo' }} Laudo</h1></div><a routerLink="/laudos" class="btn btn-outline"><span class="material-icons">arrow_back</span></a></div>
      <div class="card"><div class="card-body">
        <div class="form-grid">
          ...
          <div class="form-group full"><label>Título *</label><input class="form-control" [(ngModel)]="form.titulo"></div>
          <div class="form-group"><label>Tipo</label><select class="form-control" [(ngModel)]="form.type"><option value="LAUDO">Laudo</option><option value="PARECER">Parecer</option><option value="RELATORIO">Relatório</option></select></div>
          @if (isEdit) { <div class="form-group"><label>Status</label><select class="form-control" [(ngModel)]="form.status"><option value="RASCUNHO">Rascunho</option><option value="FINALIZADO">Finalizado</option></select></div> }
          <div class="form-group full"><label>Conteúdo *</label><textarea class="form-control" rows="15" [(ngModel)]="form.content"></textarea></div>
        </div>
        <div class="form-actions"><a routerLink="/laudos" class="btn btn-outline">Cancelar</a><button class="btn btn-primary" (click)="save()" [disabled]="saving()">{{ saving() ? 'Salvando...' : 'Salvar' }}</button></div>
      </div></div>
    </div>
  `,
...
export class LaudoFormComponent implements OnInit {
  private service = inject(LaudoService); private api = inject(ApiService); private router = inject(Router); private route = inject(ActivatedRoute);
  isEdit = false; id = ''; saving = signal(false); pacientes = signal<any[]>([]);
  form: any = { pacienteId: '', titulo: '', content: '', type: 'LAUDO', status: 'RASCUNHO' };
  ngOnInit() {
  ...
  save() {
    if (!this.form.pacienteId || !this.form.titulo || !this.form.content) return alert('Preencha os campos obrigatórios');
    this.saving.set(true);
    const obs = this.isEdit ? this.service.update(this.id, this.form) : this.service.create(this.form);
    obs.subscribe({ next: () => this.router.navigate(['/laudos']), error: () => { this.saving.set(false); alert('Erro'); } });
  }
}
