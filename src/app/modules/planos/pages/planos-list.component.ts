                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                          ...
                          <span class="text-sm font-bold text-slate-900 dark:text-white truncate">{{ p.paciente?.name || '—' }}</span>
                        </div>
                      </td>
              ...
              @for (p of items(); track p.id) {
                <div class="p-4 space-y-3">
                  <div class="flex items-start justify-between gap-3">
                    <div class="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div class="min-w-0 flex-1">
                        <h4 class="font-bold text-slate-900 dark:text-white text-sm truncate">{{ p.paciente?.name || 'Sem paciente' }}</h4>
                        <p class="text-xs text-slate-500 dark:text-slate-400">{{ p.createdAt | date:'dd/MM/yyyy' }}</p>
                      </div>
                    </div>
                  ...
                  <div class="grid grid-cols-3 gap-2 text-xs bg-slate-50 dark:bg-slate-800/50 p-2.5 rounded-2xl">
                    <div>
                      <span class="text-[10px] font-bold text-slate-500 uppercase block">Freq</span>
                      <span class="font-semibold text-slate-800 dark:text-slate-200 truncate block">{{ p.frequency || '—' }}</span>
                    </div>
                    <div>
                      <span class="text-[10px] font-bold text-slate-500 uppercase block">Sessões</span>
                      <span class="font-semibold text-slate-800 dark:text-slate-200 block">{{ p.sessionCount || 0 }}</span>
                    </div>
                    <div>
                      <span class="text-[10px] font-bold text-slate-500 uppercase block">Total</span>
                      <span class="font-bold text-primary block">{{ calculateTotal(p) | currency:'BRL' }}</span>
                    </div>
                  </div>

                  <div class="flex items-center gap-2 pt-1">
                    <a [routerLink]="['/app/planos', p.id]"
                    ...
                    <button (click)="exportPDF(p)"
                      ...
                      title="PDF">
                      <span class="material-icons text-[16px]">picture_as_pdf</span>
                    </button>
                    <a [routerLink]="['/app/planos', p.id, 'editar']"
