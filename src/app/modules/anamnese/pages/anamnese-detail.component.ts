  ...
  template: `
    <div class="page">
      <div class="header"><div><h1>Anamnese</h1><p class="subtitle">{{ item()?.paciente?.name }} — {{ item()?.createdAt | date:'dd/MM/yyyy' }}</p></div>
        <div class="actions"><a routerLink="/anamnese" class="btn btn-outline"><span class="material-icons">arrow_back</span></a><a [routerLink]="['/anamnese', id, 'editar']" class="btn btn-primary"><span class="material-icons">edit</span></a></div></div>
