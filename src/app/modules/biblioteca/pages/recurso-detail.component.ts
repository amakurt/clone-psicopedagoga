  ...
  template: `
    <div class="page">
      <div class="header"><div><h1>{{ item()?.name }}</h1><p class="subtitle">{{ item()?.category }}</p></div>
        <div class="actions"><a routerLink="/app/biblioteca" class="btn btn-outline"><span class="material-icons">arrow_back</span></a><a [routerLink]="['/app/biblioteca', id, 'editar']" class="btn btn-primary"><span class="material-icons">edit</span></a></div></div>
