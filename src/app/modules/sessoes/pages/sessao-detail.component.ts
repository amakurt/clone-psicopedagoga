  ...
  template: `
    <div class="page">
      <div class="header"><div><h1>Sessão</h1><p class="subtitle">{{ item()?.paciente?.name }} — {{ item()?.date | date:'dd/MM/yyyy HH:mm' }}</p></div>
        <div class="actions"><a routerLink="/app/sessoes" class="btn btn-outline"><span class="material-icons">arrow_back</span></a><a [routerLink]="['/app/sessoes', id, 'editar']" class="btn btn-primary"><span class="material-icons">edit</span></a></div></div>
