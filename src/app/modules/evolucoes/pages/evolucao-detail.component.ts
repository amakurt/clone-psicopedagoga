...
@Component({
  selector: 'app-evolucao-detail',
  standalone: true,
  imports: [CommonModule, RouterLink],
  template: `
    <div class="page">
      <div class="header"><div><h1>Evolução</h1><p class="subtitle">{{ item()?.paciente?.name }} — {{ item()?.date | date:'dd/MM/yyyy' }}</p></div>
        <div class="actions"><a routerLink="/evolucoes" class="btn btn-outline"><span class="material-icons">arrow_back</span></a><a [routerLink]="['/evolucoes', id, 'editar']" class="btn btn-primary"><span class="material-icons">edit</span></a></div></div>
      @if (item()) { <div class="card"><div class="card-body">
        <div class="info-grid">
          <div class="info-section"><h3>Resumo</h3><p>{{ item()?.summary || '—' }}</p></div>
          <div class="info-section"><h3>Atividades Realizadas</h3><p>{{ item()?.activities || '—' }}</p></div>
          <div class="info-section"><h3>Observações</h3><p>{{ item()?.observations || '—' }}</p></div>
          <div class="info-section"><h3>Evolução Clínica</h3><p>{{ item()?.clinicalEvolution || '—' }}</p></div>
          <div class="info-section"><h3>Conduta</h3><p>{{ item()?.conduct || '—' }}</p></div>
        </div>
      </div></div> }
    </div>
  `,
...
export class EvolucaoDetailComponent implements OnInit {
  private service = inject(EvolucoesService); private route = inject(ActivatedRoute);
